...
class EventsTimelineCard extends StatefulWidget {
  ...
  @override
  State<EventsTimelineCard> createState() => EventsTimelineCardState();
}

class EventsTimelineCardState extends State<EventsTimelineCard> {
  ...
  void scrollToEvent(String eventId) {
    ...
    if (key?.currentContext != null) {
      ...
      // Auto clear highlight after 1.2s
  ...
  Widget build(BuildContext context) {
    ...
    // 按时间升序排列
    final sortedEvents = List<TradeEvent>.from(widget.events)
          ...
          children: [
            ...
            if (sortedEvents.isEmpty)
              const Padding(
                ...
                child: Center(
                  child: Text(
                    '暂无事件。记录那些已经发生、并对原交易计划有效性产生影响的客观事实。比如本应该执行却因外部因素未执行的，本不该执行的却执行了的事件。',
              ...
              ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (!_itemKeys.containsKey(event.id)) {
                    _itemKeys[event.id] = GlobalKey();
                  }
  ...
  Widget _buildEventItem(
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: isHighlighted ? Colors.orange.withAlpha(30) : Colors.transparent,
